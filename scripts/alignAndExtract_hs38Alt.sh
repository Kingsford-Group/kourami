...
if [ ! -e "$bam_path" ] || [ ! -e "$kourami_db" ] || [ ! -e "$merged_hla_panel" ] || [ ! -e "$grch38_HLA_NoAlt" ];then
    echo "Missing one of the following files/directories (38Alt):\n"
    echo "$bam_path"
    echo "$kourami_db"
...
echo ">>>>>>>>>>>>>>>> extracting reads mapping to HLA loci and ALT contigs (38Alt)"
$samtools_bin view -b -L $resources_dir/hs38dh.initial.bed $bam_path | $samtools_bin sort --thread $num_processors -m $samtools_sort_memory_per_thread -O BAM - > $sampleid.tmp.extract.ba
...
if [ ! $OUT -eq 0 ];then
    echo 'Something went wrong while extracting HLA-related reads from hs38Alt mapping (38Alt)'
    exit 1
fi

echo ">>>>>>>>>>>>>> indexing extracted bam (38Alt)"
$samtools_bin index $sampleid.tmp.extract.bam

echo ">>>>>>>>>>>>>> bamUtil fastq extraction (38Alt)"
...
OUT=$?
if [ ! $OUT -eq 0 ];then
    echo '$bamUtil fastq extraction Failed! (38Alt)'
    exit 1
else
