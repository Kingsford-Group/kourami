...
function usage {
    ...
    echo "Note: Use this if you have bam file aligned to GRCh38 (primary assembly + [decoy] : 38)"
    ...
    echo
    echo " bamfile          : sorted and indexed bam to hs38_NoAlt or hs38D_NoAlt  (ex: NA12878.bam) [required]"
    echo
...
if [ ! -e "$bam_path" ] || [ ! -e "$kourami_db" ] || [ ! -e "$merged_hla_panel" ] || [ ! -e "$grch38_HLA_NoAlt" ];then
    echo "Missing one of the following files/directories (38):\n"
    echo "$bam_path"
    echo "$kourami_db"
    echo "$merged_hla_panel"
    echo "$grch38_HLA_NoAlt"
    exit 1
fi
...
chr6:33075926-33089696 chr6:33112516-33129113 | $samtools_bin view -b -F 0x4 - | $samtools_bin sort --thread $num_processors -m $samtools_sort_memory_per_thread -O BAM - > $sampleid.tmp.extract.bam

OUT=$?

if [ ! $OUT -eq 0 ];then
    echo 'Something went wrong while extracting HLA-related reads from hs38 mapping (38)'
    exit 1
fi

echo ">>>>>>>>>>>>>> indexing extracted bam (38)"
$samtools_bin index $sampleid.tmp.extract.bam

echo ">>>>>>>>>>>>>> bamUtil fastq extraction (38)"
...
OUT=$?
if [ ! $OUT -eq 0 ];then
    echo '$bamUtil fastq extraction Failed! (38)'
    exit 1
else
    echo rm $sampleid.tmp.extract.bam*
fi
